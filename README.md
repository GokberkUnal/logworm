...
logworm> filter --field op --when in:c,u                     # only inserts and updates (in: / notin: take comma-separated values)
...
logworm> format --set json         # pretty | table | json | line for show and tail (--format on either command overrides once)
logworm> groups                    # consumer groups with total lag
logworm> group --id my-group       # members and per-partition lag
logworm> lag --warn 100 --critical 1000    # lag thresholds (yellow / red)
logworm> watch --interval 2                # live-refresh the group table (default 10 frames; --count 0 = until Ctrl+C)
...
3. **View** — *All messages* (newest N as a table), *Only new messages* (live tail from now), or *Specific fields*: the last 50 messages are sampled for JSON keys (nested ones as `a.b`), you tick the ones to show, then choose all / new. The chosen view starts immediately after the wizard and stays the default for `show` and `tail`; `view` prints or changes it. (A live tail is not auto-started; the wizard prints a hint and you run `tail`.)
4. **Filters & alerts** — rules on one field (`key` or a JSON path picked from the sampled fields, or typed): *is-null*, *not-null*, *equals*, *not-equals*, *is-one-of*, *is-none-of* (comma-separated values), *contains*, *greater-than*, *less-than*. A **filter** hides everything that does not match (several filters are AND-ed). An **alert** colors matching rows red / yellow / green / cyan and tags them with a label (default: the condition, e.g. `price is null`); the first matching alert wins. `show` adds an `alert` column and counts matches, `tail` prefixes lines with `[label]` and reports shown / alert / hidden counts when stopped. Same rules via commands: `filter`, `alert`, `rules [--clear]`, with `--when null | notnull | eq:X | ne:X | in:A,B | notin:A,B | contains:X | gt:N | lt:N`.
5. **Output format** — *Pretty* (default): one block per message, `HH:mm:ss.SSS  key=<key>  p<partition>@<offset>` followed by one line per field (the selected fields, otherwise every JSON field of the message, or `value` for plain text). A field that differs from the previous message **with the same key** is printed as `previous --> current`. A matching alert colors the whole block and appends `[label]` to the header. Other choices: *Table* (box-drawn, one row per message), *JSON* (one compact JSON object per line: `partition`, `offset`, `timestamp`, `key`, then `value` or a `fields` object, plus `alert` when a rule matched; pipe-able to `jq`) and *Line* (`p1@1203 12:04:33.209 key  value`). `show` and `tail` use it; `tail` prints table as line. Change later with `format --set pretty|table|json|line`, or once with `show --format json` / `tail --format json`.
6. **Consumer groups** — watch all groups or a chosen subset, with a warning lag (yellow) and a critical lag (red). `groups` colors each group's total lag by those thresholds and adds a status column; `group --id X` colors per-partition lag; `watch [--interval N] [--count M] [--id X]` live-refreshes the table (clearing the screen each time) M times (default 10; `--count 0` runs until Ctrl+C). Thresholds and the watched groups change with `lag --groups a,b --warn N --critical N`. The connection is verified before the prompt appears; on failure you can retry or continue unverified. `connect --servers <addr>` switches cluster later, `connection` shows the current one. A failed switch keeps the previous connection. The shell is off in tests (`spring.shell.interactive.enabled=false`); every shell bean is guarded by `@InteractiveShellComponent` so the REST API still works headless.
