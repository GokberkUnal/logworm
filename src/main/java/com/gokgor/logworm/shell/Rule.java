...
public record Rule(String field, Condition condition, String value, Color color, String label) {
    ...
    public enum Condition {
        IS_NULL, IS_NOT_NULL, EQUALS, NOT_EQUALS, CONTAINS, GREATER_THAN, LESS_THAN;
     ...
     * {@code null}, {@code notnull}, {@code eq:X}, {@code ne:X}, {@code contains:X}, {@code gt:N}, {@code lt:N}.
     */
    public static Rule parse(String field, String when, Color color, String label) {
        ...
        if (when == null || when.isBlank()) {
            throw new IllegalArgumentException("--when is required (null, notnull, eq:X, ne:X, contains:X, gt:N, lt:N)");
        }
        String w = when.trim();
        int colon = w.indexOf(':');
